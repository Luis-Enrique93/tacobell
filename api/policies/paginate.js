  ...
  let middleware = function (req, res, next) {
    let getCount = function () {
      ...
      if (!parseBlueprintOptions) {
        req._sails.log.warn(
          `paginate middleware ignored, parseBlueprintOptions function not supported, are you sure you\'re using sails 1.0+`
        );
        ...
        .catch((err) => {
          if (!silentError) {
            req._sails.log.error(
              `[paginate] Was not able to get count for '${
                req.originalUrl
              }'\n${err.toString()}`
            );
          }
